...
min_vel = -10										#Minimun velocity of chaser
max_vel = 10										#Maximum velocity of chaser
current_position = [-2.5,0.5,34]							#Present location of chaser
errSum = [0,0,0]									#Integral sum of error
prev_position = [-2.5,0.5,34]								#Last known location of chaser
prev_vel = [0,0,0]									#Storing previous velocity value
output = [0,0,0]									#List containing magnitude of velocities in all axes

kp = 5											#Proportional gain for x,y,z error
ki = 6											#Integral gain for x,y,z error
kd = 50											#Derivative gain for x,y,z error
...
class send_data():
	...
	*Example call:	self.motion()
	...
	def motion(self):
		...
		if time.time() - caved >= 15:						#If specified time has passed
		...
		if (waypts[pt][2]+thresh <= current_position[2]):			#Checking if the drone is in positive side on z-axis
			...
			self.throttle(output[2]*12)					#Give positive PID value to drone z-axis
		elif (current_position[2] <= waypts[pt][2]-thresh):			#Checking if the drone is in positive side on z-axis
			flag[2] = -1							#Negative z error
			self.throttle(-output[2])					#Give negative PID value to drone z-axis
		...
		if flag == [0,0,0]:							#If no error in all axis
			...
			if pt >= 5:							#If last point is traversed
				print 'momentum'
		...
		prev_vel = value							#Update last velocity value
